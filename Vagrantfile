...
k3s_version = "v1.28.5+k3s1"  # Variable pour la version de K3s

coredns_fix_script = <<-SHELL
# Wait for CoreDNS to be available
echo "Waiting for CoreDNS to be ready..."
timeout 120 bash -c 'until kubectl get deployment/coredns -n kube-system &>/dev/null; do
  sleep 5
...
ping -c 3 #{master_ip} || echo "Warning: Could not ping master"
curl -vk https://#{master_ip}:6443 || echo "Warning: Could not curl master"
...
export INSTALL_K3S_VERSION="#{k3s_version}"  # Même version que le master
export INSTALL_K3S_EXEC="--node-ip=#{agent_ip} --flannel-iface=enp0s8 --resolv-conf=/etc/resolv.conf --with-node-id"

for i in {1..10}; do  # Augmentez le nombre de tentatives
  echo "Joining cluster (attempt $i)..."
  curl -sfL https://get.k3s.io | INSTALL_K3S_VERSION="#{k3s_version}" sh -s - --debug && break
  sleep 15  # Augmentez le délai entre les tentatives
done

# Verify node joined
kubectl get nodes || echo "Failed to verify node status"
SHELL
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "agent" do |agent|
    ...
    agent.vm.synced_folder ".", "/vagrant", mount_options: ["dmode=777,fmode=666"]  # Ajouté pour partager le token
